    ...
    for (ex_index,example) in enumerate(examples):
        ...
        for i, word in enumerate(textlist):
            ...
            for m in range(len(token)):
                if m >= 1 and label_1 != 'O':
                    labels.append(f"I-{label_1[2:]}")
                else:
    ...
    if args.do_train:
        ...
        for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss = model(input_ids, segment_ids, input_mask, label_ids,valid_ids)#, seq_len=seq_len)
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, f'epoch_{str(epoch_)}.pth'))
